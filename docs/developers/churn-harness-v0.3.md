...
The command runs four local integration scenarios in one Vitest worker. The main churn scenario starts a directly reachable controller and five `outbound-only` volunteers. Each volunteer initiates an authenticated relay link, and the controller selects all five over those reverse links. A TCP proxy carries one volunteer's link so the test can sever it without taking that volunteer's client mailbox offline.
...
The completion scenario publishes a matching pair, verifies five signed receipts, then stops the controller and two volunteers. A relay that never received the record forwards a search to a surviving volunteer, and a client decrypts one notice fetched from that volunteer. After the stopped relays return, the scenario checks that the same notice reaches all five volunteers, its acknowledgement converges, and a withdrawal tombstone reaches all five. The other two scenarios verify reverse-link placement against five approved volunteers while excluding a sixth, and replacement of an unavailable target by an eligible spare. All sockets run on loopback. This harness does not model real NAT devices, independent operators, Internet latency, prolonged power loss, or statistically representative churn. Repeat it across machines and realistic volunteer networks before using its timing observations as availability evidence.
