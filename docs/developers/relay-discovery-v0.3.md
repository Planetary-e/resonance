...
Next work adds compact set reconciliation, journal compaction and broader mailbox/match resource budgets, permanent-refusal target replacement, diversity evidence, graceful handoff, and query forwarding. A journal compactor must keep a tombstone with the predecessor publication state required to replay its retained record and reservation, and preserve the explicit allocation provenance or an unmarked legacy classification. Those pieces are needed to turn receipt, point-check, and allocation evidence into a measured availability claim under churn.
