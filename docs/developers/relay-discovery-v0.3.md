...
The standalone relay enables discovery when `RELAY_DISCOVERY=true` or `RELAY_PUBLIC_ENDPOINTS` contains a comma-separated list of public `ws://` or `wss://` endpoints. `RELAY_SUPPORTED_GROUPS` defaults to `public`. `RELAY_STORAGE_CAPACITY_BYTES` sets the signed capacity ceiling. `RELAY_PUBLICATION_STORAGE_QUOTA_BYTES` sets the retained-publication allocation, defaults to `RELAY_STORAGE_AVAILABLE_BYTES`, and cannot exceed it. `RELAY_REPLICA_STORAGE_PER_RELAY_BYTES` defaults to that quota and caps each inbound relay identity. A 512-byte withdrawal reserve is held inside the relay-wide quota and each per-relay cap. The descriptor's `availableBytes` is a rounded, rate-limited value from the remaining ordinary allocation. `RELAY_ADMIN_API_KEY` enables the exact `/stats` endpoint; it stays disabled when no key is configured. A relay with discovery enabled and no public endpoint advertises itself as `outbound-only`.
...
Repair can resume only when the target reconnects under the relay identity named in its receipt. A volunteer that loses both its stored data and its relay identity is a new target. Existing selected targets are attempted before the controller expands an underfull placement, so a signed divergent response in that repair pass fences the old operation before a new volunteer receives it. `capacity-exhausted` is the only response that removes a target and selects a replacement: it is issued after the receiver evaluates the exact owner-signed operation, so it cannot conceal a newer state. A signed `stale`, `conflict`, or `terminal` response is persisted as a target-specific capability for a read-only `relay_replica_reconciliation_request`; the target can return only its current owner-signed operation for that same publication or a narrow unavailable result. If the answer is an unambiguous valid successor, the source adopts it locally and retires the old placement intent without creating a new outbound intent. Same-sequence live conflicts, missing state, `unsupported-group`, and `invalid` remain quarantined. The target never replays the old placement request during reconciliation, so losing a newer state cannot resurrect it. `rate-limited` and `persistence-failed` responses retain the target for bounded retry; expired live publications are not repaired. Safe handoff remains later work.

Storage capacity remains a self-reported claim to other relays, but the receiver enforces its own retained-publication allocation before its fsynced journal write. A replica that will not fit receives a signed `capacity-exhausted` receipt and no journal record. A live owner-signed publication reserves its serialized state plus 512 bytes for its own terminal operation. Separately, each quota holds one bounded 512-byte pool that ordinary live records cannot use; a valid first-seen tombstone can use it when the relay did not retain the live predecessor. This prevents a full relay from blocking withdrawal convergence while keeping total retained state within its configured quota. New rows carry durable `local`, `replica`, or `legacy` provenance. A local tag keeps the relay's own retained state outside inbound-peer accounting even if recovery recreates the infrastructure identity; a replica tag preserves the authenticated first sender through later updates and tombstones. Historical unmarked rows are never guessed from the owner signature or timing: they use one internal legacy allocation bucket, which is subject to the per-relay cap but excluded from the named `replica_storage_relays` count. A new update or tombstone of that historical state is written with the explicit legacy tag, so compaction retains the classification. Exact local or authorized statistics report its bytes as `legacy_unattributed_storage_reserved_bytes`; the relay logs a startup warning while it is nonzero. Natural expiry removes a record from matching but does not release retained-state allocation; it may still be renewed or tombstoned. The accounting pool deliberately excludes historical journal bytes, matches, and mailbox contents until compaction and broader resource budgeting exist. A public descriptor is a coarse placement hint; signed receipt responses are authoritative. Operators must not use the five-target count as a Sybil-resistance or availability guarantee.

## Next integration step

Next work adds compact set reconciliation, journal compaction and broader mailbox/match resource budgets, diversity evidence, graceful handoff, and query forwarding. A journal compactor must keep a tombstone with the predecessor publication state required to replay its retained record and reservation, and preserve the explicit allocation provenance or an unmarked legacy classification. Those pieces are needed to turn receipt, point-check, and allocation evidence into a measured availability claim under churn.
