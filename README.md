...
To expose signed v0.3 discovery metadata, set `RELAY_PUBLIC_ENDPOINTS` to the relay's comma-separated public WebSocket endpoints. Use `RELAY_CONTACTS` for configured relay hints; every contacted relay and every descriptor it returns is verified independently. See [Relay discovery for v0.3](docs/developers/relay-discovery-v0.3.md) for all discovery settings and their trust model.
