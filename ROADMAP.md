...
- [ ] Detect and advertise relay capabilities such as inbound reachability, storage capacity, supported groups, and forwarding availability
  ...
  - [ ] Verify advertised direct endpoints from an independent network before presenting them as observed Internet reachability; a successful local or single-peer connection is only evidence for that path
