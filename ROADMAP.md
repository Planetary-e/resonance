...
- [ ] Publish to a remote placement set even when the personal node also operates a local relay
  ...
  - [ ] Support replica placement, receipt checks, mailbox repair, and reconciliation over an inbound link initiated by an outbound-only volunteer, so a device behind NAT can be selected as a target
...
- [ ] Give pairwise relationship mailboxes, which are not attached to a publication, an explicit volunteer placement and recovery policy
