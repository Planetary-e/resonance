...
**Current replication boundary:** a receipt proves that a named relay fsynced one exact operation at one point in time. A receipt-authorized point check can add a recent signed `present` or `missing` answer for that exact operation and repairs a genuine loss when that relay returns with the same infrastructure identity. Volunteer relays now enforce a retained-state quota and publish a coarse, rate-limited allocation hint, but that is not a filesystem-free-space guarantee while the append-only journal remains un-compacted. New journal rows carry explicit `local`, `replica`, or `legacy` provenance, preserving local accounting through relay identity recovery; old unmarked rows share one conservative legacy per-relay bucket rather than being guessed. Only a signed `capacity-exhausted` refusal can replace a target, because it follows current-state evaluation. A signed `stale`, `conflict`, `terminal`, `unsupported-group`, or `invalid` refusal instead durably quarantines all automatic fan-out of that exact operation; a new owner-signed revision starts fresh placement. Neither a receipt nor a capacity hint proves independence or continuous availability, and an identity reset is not yet a replacement signal. Compact inventory reconciliation, journal compaction, peer-diversity evidence, graceful handoff, and broader resource limits remain required before Resonance can claim replica health under churn. Journal compaction must retain a tombstone together with the predecessor publication state and allocation provenance it needs for replay and accounting.
